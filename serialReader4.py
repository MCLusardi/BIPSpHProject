#Python code for connecting four Arduinos or DFRduinos to RasPi
...
#The purpose is to read data from four Arduino Megas or DFRduinos receiving data from any number of sensors 
...
#Once you know the port number and baudrate, you enter them into the serial functions in lines 27, 29, 31, 33
...
def main_func():
    ...
    arduino3 = serial.Serial('/dev/ttyACM0', 115200)
    print('Established serial connection to arduino3')
    arduino4 = serial.Serial('/dev/ttyACM0', 115200)
    print('Established serial connection to arduino4')
    ...
    arduino_data4 = arduino4.readline()
    ...
    decoded_values4 = str(arduino_data4[0:len(arduino_data4)-2].decode("utf-8"))

    print('Collected readings from Arduino: {}, {}, {}, {}'.format(decoded_values1, decoded_values2, decoded_values3, decoded_values4))
    file.write(time.asctime(time.localtime(time.time())))
    file.write(',')
    file.write(decoded_values1 + ',' + decoded_values2 + ',' + decoded_values3 + ',' + decoded_values4)
    file.write('\n')
    file.close()
