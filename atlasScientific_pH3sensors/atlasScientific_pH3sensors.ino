...
// This code is intened to read values from three Atlas Scientific pH micro sensor via Serial communication
// To work, connect the sensors to serial port 3 (pins 14 and 15 on Arduino Mega), serial port 2 (pins 16 and 17), and serial port 1 (pins 18 and 19)
...
float pH3;                                            //used to hold a floating point number that is pH3
...
String pHsensorstring1 = "";                          // ditto for pH sensor 1
boolean pHinput_string_complete1 = false;
boolean pHsensor_string_complete1 = false;
float pH1;
...
void serialEvent1() {
  pHsensorstring1 = Serial1.readStringUntil(13);
  pHsensor_string_complete1 = true;
}
...
void loop() {
  if (input_string_complete == true) {                //if a string from the PC has been received in its entirety
    ...
    else if(sensorType.compareTo("s1:") == 0){
      //Serial.print("Sending command to PH: ");
      //Serial.println(command);
      
      Serial1.print(command);                         //send that command to that sensor
      Serial1.print('\r'); 
    }
...
// the commented out functions below would print data from just s3, s2, or s1
  ...
  if (pHsensor_string_complete1 == true) {            //if a string from the Atlas Scientific product has been received in its entirety
    Serial.print("s1,");
    Serial.println(pHsensorstring1);                    //send that string to the PC's serial monitor
    pHsensorstring1 = "";                               //clear the string:
    pHsensor_string_complete1 = false;                //reset the flag used to tell if we have received a completed string from the Atlas Scientific product
  }
*/

// if both s3, s2, and s1 have sent data, this funciton will print each value in the order "s3, s2, s1"
  if (pHsensor_string_complete3 == true && pHsensor_string_complete2 == true) {
    ...
    Serial.print(',');
    Serial.print(pHsensorstring2);
    Serial.print(',');
    Serial.println(pHsensorstring1);
    pHsensorstring3 = "";
    pHsensor_string_complete3 = false;
    pHsensorstring2 = "";
    pHsensor_string_complete2 = false;
    pHsensorstring1 = "";
    pHsensor_string_complete1 = false;
  }
}
